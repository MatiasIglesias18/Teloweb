...
import solicitarUpdateFotosTipoHab from "@/app/utils/manejoTipoHabitaciones/solicitarUpdateFotosTipoHab";

import { collection, query, where, getDocs } from "firebase/firestore";
import { db } from "@/firebase/config";

const TipoHabCargarImagenesForm = () => {
  const [procesandoForm, setProcesandoForm] = useState(false);
  const [openDialog, setOpenDialog] = useState(false);
  const [cambiosPendientes, setCambiosPendientes] = useState(false);
  const { tipoHabitacionDoc, tipoHabitacionRef } = useTipoHabContext();
  ...
  useEffect(() => {
    (async () => {
      const queryCambios = query(
        ...
        where("docUid", "==", tipoHabitacionRef.id),
        where("estado", "==", "pendiente"),
        where("type", "==", "fotosTipoHab")
      );
  ...
  const handleSubirImagenes = async (e) => {
    ...
    const [result, error] = await solicitarUpdateFotosTipoHab(
      teloRef,
      tipoHabitacionRef,
      filesArray
    );
...
export default TipoHabCargarImagenesForm;
